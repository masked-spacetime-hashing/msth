...
class _grid_encode(Function):
    @staticmethod
    @custom_fwd
    def forward(ctx, inputs, embeddings, offsets, per_level_scale, base_resolution, calc_grad_inputs=False, gridtype=0, align_corners=False, interpolation=0, grid_mask=None):
...
class GridEncoder(nn.Module):
    ...
    def upsample(self, resolution=None):
        ...
        upsampled_embeddings = F.interpolate(
            ...
            size=(resolution if self.align_corners else resolution + 1, 
    ...
    def forward(self, inputs, bound=1):
        ...
        outputs = grid_encode(inputs, self.embeddings, self.offsets, self.per_level_scale, self.base_resolution, inputs.requires_grad, self.gridtype_id, self.align_corners, self.interp_id, self.grid_mask)
    ...
    def hash_reinitialize(self, inputs, std=1e-4):
        ...
        inputs = inputs.view(-1, self.input_dim)
        B = inputs.shape[0]
        grid_encode_hash_reinitialize(inputs, self.embeddings, self.offsets, B, self.input_dim, 
                                      self.embeddings.shape[1], self.offsets.shape[0]-1, 
                                      np.log2(self.per_level_scale), self.base_resolution, self.gridtype_id, 
                                      self.align_corners, 0, std, self.grid_mask)
    ...
    def generate_grid_mask(self, inputs, bound=1):
        ...
        B = inputs.shape[0]
        # print(inputs)
        grid_encode_set_static(inputs, self.grid_mask, self.offsets, B, self.input_dim, 
                                      self.embeddings.shape[1], self.offsets.shape[0]-1, 
                                      np.log2(self.per_level_scale), self.base_resolution, self.gridtype_id, 
                                      self.align_corners)
