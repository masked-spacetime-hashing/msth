#!/opt/czl/anaconda3/envs/MSTH/bin/python python
...
import tyro
import yaml
from configs.method_configs import AnnotatedBaseConfigUnion
...
from trainer import VideoTrainerConfig
...
def train_loop(local_rank: int, world_size: int, config: VideoTrainerConfig, global_rank: int = 0):
...
def launch(
    ...
    machine_rank: int = 0,
    dist_url: str = "auto",
    config: Optional[VideoTrainerConfig] = None,
    timeout: timedelta = DEFAULT_TIMEOUT,
) -> None:
...
def main(config: VideoTrainerConfig) -> None:
    """Main function."""
