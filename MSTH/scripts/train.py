...
from MSTH.configs.method_configs import AnnotatedBaseConfigUnion
...
def train_loop(local_rank: int, world_size: int, config: TrainerConfig, global_rank: int = 0):
    ...
    trainer.setup()
    trainer.train()
    trainer.pipeline.get_eval_video(30)
    trainer.pipeline.render_from_cameras(
        1.0,
        5.0,
        save_path=f"./spirals_{wandb.run.name}.mp4",
        fps=30,
        num_frames=30,
        offset=config.render_camera_offset,
    )
