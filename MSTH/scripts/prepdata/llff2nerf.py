...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path", type=str, help="root directory to the LLFF dataset (contains videos / and pose_bounds.npy)"
    )
    parser.add_argument(
        "--videos",
        type=str,
        default="videos",
        help="videos folder (do not include full path, e.g., just use `videos`)",
    )
    parser.add_argument("--downscale", type=float, default=2, help="image size down scale, e.g., 4")
    parser.add_argument("--num_frames", type=int, default=300, help="image size down scale, e.g., 4")
    parser.add_argument("--hold", type=int, default=None, help="hold out for validation every $ videos")
    parser.add_argument("--hold_list", type=int, default=0, nargs="+")
    ...
    videos = [
        f[len(opt.path) :]
        for f in sorted(glob.glob(os.path.join(opt.path, opt.videos, "*")))
        if f.lower().endswith("mp4") or f.lower().endswith("mp4") or f.lower().endswith("mp4")
    ]
    print(videos)
    ...
    print(f"[INFO] loaded {len(videos)} videos, {N} poses_bounds as {poses_bounds.shape}")
    ...
    all_ids = np.arange(N)
    if opt.hold_list is not None:
        test_ids = opt.hold_list
    else:
    ...
    for i in train_ids:
        frames_train.append(
            {
                "file_path": os.path.basename(videos[i]),
    ...
    def write_json(filename, frames):
        ...
        # write
        output_path = os.path.join(opt.path, opt.videos, filename)
        print(f"[INFO] write {len(frames)} videos to {output_path}")
