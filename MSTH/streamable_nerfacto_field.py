...
class HashConsistencyModule(nn.Module):
    def __init__(self, features_per_level, num_levels, geo_feat_dim, hidden_dim, num_layers):
        super().__init__()
        self.features_per_level = features_per_level
        self.MLP = tcnn.Network(
            n_input_dims=(num_levels + 2) * features_per_level,
    ...
    def forward(self, features):
        ...
        mean = features.mean(dim=1).unsqueeze(dim=1)
        std = features.std(dim=1).unsqueeze(dim=1)/100
        input = torch.cat([mean, std, features], dim=1)
        input = rearrange(input, 'b l f -> b (l f)')
...
class TCNNStreamableNerfactoField(Field):
    ...
    def __init__(
        ...
        self.mlp_after_encoding = HashConsistencyModule(features_per_level, num_levels, self.geo_feat_dim, hidden_dim, num_layers)
        # tcnn.Network(
    ...
    def disable_MLP(self):
        # for param in self.mlp_after_encoding.parameters():
        #     param.requires_grad = False

        for param in self.mlp_head.parameters():
            param.requires_grad = False

    def hash_reinitialize(self, ray_samples: RaySamples) -> None:
        ...
        self.encoding.hash_reinitialize(positions_flat)
...
field_implementation_to_class: Dict[str, Field] = {"tcnn": TCNNStreamableNerfactoField}
