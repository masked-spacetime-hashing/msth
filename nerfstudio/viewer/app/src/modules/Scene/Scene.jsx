...
export function get_scene_tree() {
  ...
  const onMouseClick = (event) => {
    // TODO: check here for calculating the accurate position of mouse cursor
    if (!(event.target.nodeName === "CANVAS")) {
      return;
    }
    const state = store.getState().renderingState
    ...
    const pointObj = new CSS2DObject(point);
    pointObj.position.set(mouseX / 1000.0, mouseY / 1000.0, 0);
    scene.add(pointObj);

    if (state.use_sam) {
      store_dispatch(
        "renderingState/xs",
        state.xs.concat([mouseX]),
      );
      store_dispatch(
        "renderingState/ys",
        state.ys.concat([mouseY]),
      );
      window.points.push(pointObj);
    }

    // console.log(state.xs);
    // console.log(state.ys);
