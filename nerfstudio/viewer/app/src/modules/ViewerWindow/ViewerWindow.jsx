...
export default function ViewerWindow(props) {
  ...
  const img_x_pos = useSelector(
    (state) => state.renderingState.img_x_pos,
  );
  const img_y_pos = useSelector(
  ...
  const sendCamera = () => {
    if (websocket.readyState === WebSocket.OPEN) {
      ...
      const sam_data = {
        type: 'write',
        path: 'renderingState/sam_data',
        data: sam_data_packet,
      };
      const sam_message = msgpack.encode(data);
      websocket.send(message);
    }
  };
  ...
  return (
    <>
      ...
      <WebRtcWindow />
      <div className="canvas-container-main" ref={myRef} id="main-canvas">
